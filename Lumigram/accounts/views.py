...
def profile_view(request, username):
    ...
    if request.method == 'PUT':
        ...
        profile.is_private = request.data.get(
            'is_private',
            str(profile.is_private)
        ).lower() == 'true'

        profile.push_notifications = request.data.get(
            'push_notifications',
            str(profile.push_notifications)
        ).lower() == 'true'

        profile.email_notifications = request.data.get(
            'email_notifications',
            str(profile.email_notifications)
        ).lower() == 'true'
...
def profile_page(request, username):
    ...
    return render(request, 'profile.html', {
        ...
        'following_count': User.objects.filter(
            profile__followers=profile_user
        ).count(),
